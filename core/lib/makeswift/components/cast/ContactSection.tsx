...
const ContactSection = forwardRef(function ContactSection(
  ...
  return (
    <section
      ...
      style={{ width: "100%", background: lightMode ? '#F5F5F5' : (bgColor || "#0d1620") }}
    >
      <style>{`
        ...
        .cs-radio-label { display: flex; align-items: center; gap: 8px; cursor: pointer; font-family: 'Barlow', sans-serif; font-size: 14px; color: ${lightMode ? '#0D1620' : 'rgba(255,255,255,0.8)'}; padding: 8px 14px; border: 1px solid ${lightMode ? '#d0d0d0' : 'rgba(255,255,255,0.12)'}; border-radius: 8px; transition: border-color 200ms ease, background 200ms ease; }
        .cs-radio-label:has(input:checked) { border-color: #007CB0; background: rgba(0,124,176,0.12); color: ${lightMode ? '#0D1620' : '#fff'}; }
        .cs-radio-label input { accent-color: #007CB0; }
        .cs-select { width: 100%; padding: 10px 14px; background: ${lightMode ? '#FFFFFF' : 'rgba(255,255,255,0.06)'}; border: 1px solid ${lightMode ? '#d0d0d0' : 'rgba(255,255,255,0.12)'}; border-radius: 8px; color: ${lightMode ? '#0D1620' : '#fff'}; font-family: 'Barlow', sans-serif; font-size: 15px; box-sizing: border-box; appearance: none; }
        .cs-select option { background: ${lightMode ? '#fff' : '#1a2332'}; color: ${lightMode ? '#0D1620' : '#fff'}; }
          ...
          <div style={{ background: lightMode ? '#FFFFFF' : "#1a2332", border: lightMode ? "1px solid #d0d0d0" : "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "40px 36px" }}>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                {/* I Am A... */}
                <div style={fieldWrap}>
                  <label style={activeLabelStyle}>I Am A… *</label>
                ...
                {/* Company */}
                <div style={fieldWrap}>
                  <label style={activeLabelStyle}>Company or Organization</label>
                  <input type="text" value={company} onChange={e => setCompany(e.target.value)} placeholder="ACME Landscaping" style={activeInputStyle} />
                </div>
                ...
                <div style={fieldWrap}>
                  ...
                  <input type="email" value={emailVal} onChange={e => setEmailVal(e.target.value)} placeholder="john@company.com" required style={activeInputStyle} />
                </div>

                {/* Phone + Zip */}
                <div className="cs-name-row">
                  <div style={fieldWrap}>
                    <label style={activeLabelStyle}>Best Phone Contact *</label>
                    <input type="tel" value={phoneVal} onChange={e => setPhoneVal(e.target.value)} placeholder="(555) 000-0000" required style={activeInputStyle} />
                  </div>
                  <div style={fieldWrap}>
                ...
                {/* Help topic */}
                <div style={fieldWrap}>
                  <label style={activeLabelStyle}>What can we help you with? *</label>
                ...
                {/* Newsletter */}
                <div style={fieldWrap}>
                  <label style={activeLabelStyle}>Sign up for our monthly newsletter?</label>
                  <div className="cs-radio-group">
                ...
                {/* Message */}
                <div style={fieldWrap}>
                  <label style={activeLabelStyle}>Let us know how we can help *</label>
